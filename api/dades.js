...
const { xemaValues } = require('./_xema');

const HEADERS = { 'User-Agent': 'Mozilla/5.0 (PROJECTE-METEO)', 'X-Requested-With': 'XMLHttpRequest' };
const wc = stations.filter((s) => s.src === 'wc');
const xema = stations.filter((s) => s.src === 'xema');

module.exports = async (req, res) => {
  const [results, xv] = await Promise.all([
    Promise.all(wc.map(async (s) => {
      try {
  ...
  const values = { ...Object.fromEntries(results.filter(([, v]) => v)), ...xv };
